...
![](/asserts/2015-04-22-linux012-mm-distribution.png)
...
![](/asserts/2015-04-22-linux012-mm-dir.png)

页表项数据，P=1时表示该页数据在内存中。P=0表示该页数据在swap中，并且前31bit代表的swap_nr，即映射都硬盘位置的标号。
![](/asserts/2015-04-22-linux012-mm-direntry.png)
...
如果共享不成功(没有任务打开inode，或者对应inode的这块线性地址并没有在物理内存中)，则根据线性地址计算出在inode中的block号，读出4个block(1 block = 1K)到物理页page中，并把超过end_data的置0，最后将address的线性地址和读出的物理页page关联(put_page) 
...
![](/asserts/2015-04-22-linux012-mm-bucket_dir.png)
...
![](/asserts/2015-04-22-linux012-mm-layers.png)
